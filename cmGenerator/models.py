...
class Club(models.Model):
    ...
    Attributes:
        league (Competition): The league in which the club competes. ForeignKey to Competition model 
            with CASCADE delete behavior. When a competition is deleted, all associated clubs will 
            also be deleted.
        name (str): The name of the club. CharField with max_length=255, indexed for faster queries.
            Must be unique within each country.
        club_logo_small_url (str): URL to the small version of the club's logo. URLField that can 
            be blank or null.
        club_logo_big_url (str): URL to the big version of the club's logo. URLField that can be 
            blank or null.
        overall (int): The overall rating of the club on a scale from 1 to 99, validated using 
            MinValueValidator and MaxValueValidator.
        att_rating (int): The attacking rating of the club on a scale from 1 to 99, validated 
            using MinValueValidator and MaxValueValidator.
        mid_rating (int): The midfield rating of the club on a scale from 1 to 99, validated 
            using MinValueValidator and MaxValueValidator.
        def_rating (int): The defensive rating of the club on a scale from 1 to 99, validated 
            using MinValueValidator and MaxValueValidator.
        country (str): The country where the club is based. CharField with max_length=100, 
            indexed for faster queries.
        scout_region (str): The geographic region where the club can scout players. CharField 
            with max_length=100.
        dom_prestige (int): The domestic prestige of the club on a scale from 1 to 10, 
            validated using MinValueValidator and MaxValueValidator.
        intl_prestige (int): The international prestige of the club on a scale from 1 to 10, 
            validated using MinValueValidator and MaxValueValidator.
        league_rep (int): The league reputation of the club on a scale from 1 to 10, 
            validated using MinValueValidator and MaxValueValidator.
        youth_scouting_region (str): The geographic region where the club can scout youth 
    ...
    Meta:
        ...
        constraints (list): Database constraints to ensure data integrity:
            - unique_club_name_per_country: Ensures club names are unique within each country
    """
    
    # Existing fields
    league = models.ForeignKey(Competition, on_delete=models.CASCADE)
    name = models.CharField(max_length=255, db_index=True)  # Add index for faster queries
    ...
    country = models.CharField(max_length=100, db_index=True)  # Add index for faster queries
...
class Player(models.Model):
    """
    Represents a football player with detailed personal, professional, and performance attributes.

    Attributes:
        Basic Information:
            player_id (int): Unique identifier for the player. IntegerField with unique=True.
            name (str): Player's full name. CharField(100) with db_index for faster queries.
            slug (str): URL-friendly version of name. SlugField(150), auto-generated, unique.
            positions (list): Array of up to 3 positions from POSITION_CHOICES. ArrayField with CharField(3).
            nationality (str): Player's country of origin. CharField(100).
            
        Personal Details:
            birth_date (date): Player's date of birth. DateField.
            birth_year (int): Year of birth, derived from birth_date. IntegerField, can be null.
            age (int): Current age, consider using current_age property instead. IntegerField.
            face_pic_url (str): URL to player's profile image. URLField, can be null.
        ...
        Club Information:
            ...
            contract_loan (bool): Indicates if player is on loan. BooleanField, default False.
        ...
        Ratings:
            overall (int): Current ability rating (1-99). IntegerField with validators.
            potential (int): Maximum potential ability rating (1-99). IntegerField with validators.

        Import Data:
            last_import_date (datetime): Last data update timestamp. DateTimeField(auto_now=True).
            import_source (str): Data source identifier. CharField(100), can be null.
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                check=models.Q(contract_end__gt=models.F('contract_start')),
                name='contract_end_after_start'
            ),
            # Ensure birth_year matches birth_date
            models.CheckConstraint(
                check=models.Q(birth_year=models.ExtractYear('birth_date')),
                name='birth_year_matches_date'
            ),
            # Ensure wages are positive
            models.CheckConstraint(
                check=models.Q(wage_eur__gt=0) & models.Q(wage_usd__gt=0) & models.Q(wage_gbp__gt=0),
                name='positive_wages'
            ),
...
class Story(models.Model):
    """
    Represents a user's career mode story with enhanced tracking and customization options.
...
class Season(models.Model):
    ...
    season_number = models.PositiveIntegerField(
        help_text="Sequential number of this season in the story (1, 2, 3, etc.)"
    )
...
class PlayerStats(models.Model):
    ...
    Attributes:
        story (Story): The story to which these statistics belong. ForeignKey to the Story model with CASCADE delete behavior.
        season (Season): The season during which these statistics were recorded. ForeignKey to the Season model with CASCADE delete behavior.
        player (Player): The player to whom these statistics belong. ForeignKey to the Player model with CASCADE delete behavior.
        overall_rating (int): The player's overall rating for this season. IntegerField with a default value of 0, validated to be between 0 and 99.
        appearances (int): The total number of appearances made by the player across all competitions. IntegerField with a default value of 0, validated to be non-negative.
        goals (int): The total number of goals scored by the player. IntegerField with a default value of 0, validated to be non-negative, and indexed for sorting.
        assists (int): The total number of assists made by the player. IntegerField with a default value of 0, validated to be non-negative.
        clean_sheets (int): The total number of clean sheets kept by the player (mainly for goalkeepers). IntegerField with a default value of 0, validated to be non-negative.
        red_cards (int): The number of red cards received by the player. IntegerField with a default value of 0, validated to be non-negative.
        yellow_cards (int): The number of yellow cards received by the player. IntegerField with a default value of 0, validated to be non-negative.
        average_rating (decimal): The player's average match rating out of 10. DecimalField with a maximum of 4 digits and 2 decimal places, default value of 0.00, validated to be between 0 and 10, and indexed for sorting.

    Methods:
        aggregate_competition_stats(): Aggregates and returns the player's statistics from all competitions in the season.
        goals_per_game(): Calculates and returns the player's goals per game ratio.
        assists_per_game(): Calculates and returns the player's assists per game ratio.
        update_from_competitions(): Updates the player's aggregate statistics from competition statistics.
    ...
    def aggregate_competition_stats(self):
        ...
        total_clean_sheets = sum(stat.clean_sheets for stat in competition_stats)
        total_red_cards = sum(stat.red_cards for stat in competition_stats)
        total_yellow_cards = sum(stat.yellow_cards for stat in competition_stats)
        average_rating = sum(stat.average_rating for stat in competition_stats) / len(competition_stats) if competition_stats else 0
    ...
    def goals_per_game(self):
        ...
        return round(self.goals / self.appearances, 2) if self.appearances > 0 else 0
    
    @property
    def assists_per_game(self):
        """Calculate assists per game ratio"""
        return round(self.assists / self.appearances, 2) if self.appearances > 0 else 0
...
class CompetitionPlayerStats(models.Model):
    ...
    Attributes:
        story (Story): The story to which these statistics belong. ForeignKey to the Story model with CASCADE delete behavior.
        season (Season): The season during which these statistics were recorded. ForeignKey to the Season model with CASCADE delete behavior.
        competition (Competition): The competition in which these statistics were recorded. ForeignKey to the Competition model with CASCADE delete behavior.
        player (Player): The player to whom these statistics belong. ForeignKey to the Player model with CASCADE delete behavior.
        overall_rating (int): The player's overall rating in this competition. IntegerField with a default value of 0, validated to be between 0 and 99.
        appearances (int): The total number of appearances made by the player in this competition. IntegerField with a default value of 0, validated to be non-negative.
        goals (int): The total number of goals scored by the player in this competition. IntegerField with a default value of 0, validated to be non-negative, and indexed for sorting.
        assists (int): The total number of assists made by the player in this competition. IntegerField with a default value of 0, validated to be non-negative.
        clean_sheets (int): The total number of clean sheets kept by the player in this competition (mainly for goalkeepers). IntegerField with a default value of 0, validated to be non-negative.
        red_cards (int): The number of red cards received by the player in this competition. IntegerField with a default value of 0, validated to be non-negative.
        yellow_cards (int): The number of yellow cards received by the player in this competition. IntegerField with a default value of 0, validated to be non-negative.
        average_rating (decimal): The player's average match rating out of 10 in this competition. DecimalField with a maximum of 4 digits and 2 decimal places, default value of 0.00, validated to be between 0 and 10, and indexed for sorting.

    Methods:
        goals_per_game(): Calculates and returns the player's goals per game ratio for this competition.

    Meta:
        unique_together (tuple): Ensures that the combination of season, competition, and player is unique.
        indexes (list): Database indexes for optimizing queries on frequently accessed fields.
    """
    story = models.ForeignKey(Story, on_delete=models.CASCADE, related_name='competition_player_stats')
    season = models.ForeignKey(Season, on_delete=models.CASCADE, related_name='competition_player_stats')
...
class CompetitionWinner(models.Model):
    ...
    Attributes:
        story (Story): The story to which the competition winner belongs. ForeignKey to the Story model with CASCADE delete behavior.
        season (Season): The season during which the competition was held. ForeignKey to the Season model with CASCADE delete behavior.
        competition (Competition): The competition that was won. ForeignKey to the Competition model with CASCADE delete behavior.
        winner (Club): The club that won the competition. ForeignKey to the Club model with CASCADE delete behavior.
    ...
    def __str__(self):
        return f"{self.competition.name} - {self.season.name} Winner: {self.winner.name}"
